...
trans_1 = transforms.Compose(
            ...
            d_utils.PointcloudRandomCrop(p=1.0, min_num_points=2048),
            d_utils.PointcloudNormalize(),
            d_utils.PointcloudRandomCutout(p=1.0, min_num_points=2048),
...
trans_2 = transforms.Compose(
            ...
            [
                d_utils.PointcloudToTensor(),
                d_utils.PointcloudUpSampling(max_num_points=4096, centroid="random"),
                d_utils.PointcloudRandomCrop(p=0.1, min_num_points=2048),
                d_utils.PointcloudNormalize(),
                d_utils.PointcloudRandomCutout(p=0.1, min_num_points=2048),
                d_utils.PointcloudScale(p=1),
                # d_utils.PointcloudRotate(p=1, axis=[0.0, 0.0, 1.0]),
                d_utils.PointcloudRotatePerturbation(p=1),
                d_utils.PointcloudTranslate(p=0.1),
                d_utils.PointcloudJitter(p=1),
                d_utils.PointcloudRandomInputDropout(p=1),
                d_utils.PointcloudSample(num_pt=1024)
            ]
...
trans_3 = transforms.Compose([
    d_utils.PointcloudToTensor(),
    d_utils.PointcloudTranslate(p=0.5),
    d_utils.PointcloudJitter(p=1),
    d_utils.PointcloudNormalize(),
    d_utils.PointcloudSample(num_pt=128)
])
...
def load_modelnet_data(partition):
    BASE_DIR = '../'
    DATA_DIR = os.path.join(BASE_DIR, 'datasets/modelnet40')
    all_data = []
    all_label = []
...
class ModelNet40(Dataset):
    def __init__(self, num_points, partition='train' ,local=0):
    ...
    def __getitem__(self, item):
        ...
        if self.partition == 'train':
            ...
            # point_t1 = point_t1.transpose(1, 0)
            crops.append(point_t1)
            point_t2 = trans_2(point_cloud)
            # point_t2 = point_t2.transpose(1, 0)
            crops.append(point_t2)
            for _ in range(self.local):
                point_temp = trans_3(point_cloud)
                # point_temp = point_temp.transpose(1, 0)
                crops.append(point_temp)
